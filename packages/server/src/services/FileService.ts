...
export class FileService {
  ...
  async createFile(
    ...
    const defaultName = path.basename(abs);
    const uniqueName = await resolveUniqueName(dirAbs, defaultName, "file");
    const finalAbs = path.join(dirAbs, uniqueName);
    const finalRelative = path.relative(assetsRoot(projectId), finalAbs);

    await fsp.mkdir(dirAbs, { recursive: true });
    await fsp.writeFile(finalAbs, content, { flag: "wx" });
    await metaService.create(projectId, finalRelative);
  }
